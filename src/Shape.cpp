...
void Shape::init()
{
	...
	if (norBuf.empty())
	{
		...
		norBuf.resize(posBuf.size(), 1.0);

		//cout << "verts\n" << posBuf[0] << "\n" << posBuf[1] << "\n" << posBuf[2] << "\n";
		//cout << "normss\n" << norBuf[0] << "\n" << norBuf[0] << "\n" << norBuf[0] << "\n";

		for (int i = 0; i <= (eleBuf.size()/3); i += 3) {
			
			v1 = glm::vec3(posBuf[(eleBuf[i])*3], posBuf[(eleBuf[i])*3 + 1], posBuf[(eleBuf[i])*3 + 2]);
			v2 = glm::vec3(posBuf[(eleBuf[i+ 1])*3], posBuf[(eleBuf[i + 1])*3 + 1], posBuf[(eleBuf[i + 1])*3 + 2]);
			v3 = glm::vec3(posBuf[(eleBuf[i + 2])*3], posBuf[(eleBuf[i + 2])*3 + 1], posBuf[(eleBuf[i + 2])*3 + 2]);
			...
			norBuf[eleBuf[i]*3] += norm.x;
			norBuf[eleBuf[i]*3 + 1] += norm.y;
			norBuf[eleBuf[i]*3 + 2] += norm.z;

			norBuf[eleBuf[i + 1]*3] += norm.x;
			norBuf[eleBuf[i + 1]*3 + 1] += norm.y;
			norBuf[eleBuf[i + 1]*3 + 2] += norm.z;
			

			norBuf[eleBuf[i + 2]*3] += norm.x;
			norBuf[eleBuf[i + 2]*3 + 1] += norm.y;
			norBuf[eleBuf[i + 2]*3 + 2] += norm.z;
		}
